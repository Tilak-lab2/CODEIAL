...
module.exports.render_email_page = (req, res) =>
{
    if (req.user)//user should not be logged in before accessing this page or using this action
    {
        return res.redirect('back');
    }
    return res.render('forgot_password', { title: 'Comspace Express' });
}
...
module.exports.redirect_to_change_password_page = async function (req, res)
{
    ...
    try
    {
        ...
        return res.render('change_pass', { title: 'CODEIAL | Change Password', access_token: token_in_link });
    }
    catch (error)
    {
        if (error)
